class Reservation < ApplicationRecord
  ...
  def between_different_people
    self.guest != self.accommodation.administrator
  end
  
# Méthode qui complète overlaping_reservation? (Accommodation) et qui est appelée dans la méthode is_not_overlaping
 
  def surrounds?
    surrounded_reservations = []
    self.accommodation.reservations.each do |r|
  ...
  def is_not_overlaping
    self.accommodation.overlaping_reservation?(self.start_date) == false &&
    self.accommodation.overlaping_reservation?(self.end_date) == false && 
    surrounds? == false
  end
end